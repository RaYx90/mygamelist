<template>
  ...
  <GameDetailModal
    ...
    :fav-count="(gameStatus?.myFavorites?.includes(selectedGame.gameId) ? 1 : 0) + (gameStatus?.favoritedByInGroup?.[selectedGame.gameId]?.length ?? 0)"
    :favorited-by="gameStatus?.favoritedByInGroup?.[selectedGame.gameId] ?? []"
